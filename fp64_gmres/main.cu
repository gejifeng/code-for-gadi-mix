...
int main(int argc, char* argv[]) {
    ...
    FP64GMRES::OrthogonalizationMethod method = FP64GMRES::MGS;
    if (argc >= 3) {
        std::string method_str = argv[2];
        if (method_str == "cgs" || method_str == "CGS") {
            method = FP64GMRES::CGS;
    ...
    size_t bufferSize;
    void* d_buffer;
    cusparseSpMV_bufferSize(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, matA, vecX, &beta, vecB,
                            CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize);
    cudaMalloc(&d_buffer, bufferSize);
    cusparseSpMV(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, matA, vecX, &beta, vecB,
    ...
    printf("Starting FP64 GMRES...\n");
    FP64GMRES solver(n, nnz, A_dev);
    ...
    cusparseDnVecDescr_t vecX_final;
    cusparseDnVecDescr_t vecR_final;
