...
# Configuration
train = [16, 32, 48, 64, 96, 108, 128, 180, 256, 320, 360, 400, 512]
# N_START = 12
# N_END = 64
# N_STEP = 4
W = 0.1
...
def compile_code():
    ...
    subprocess.run(["make"], cwd="exp1", check=True)

def generate_matrix(n):
    print(f"Generating matrix for N={n}...")
    # python exp1/generate_A.py <N> <EPSILON>
    script_path = os.path.join("exp1", "generate_A.py")
    subprocess.run(["python", script_path, "-n", str(n)], check=True)
    return os.path.join("exp1", f"sylve{n}.mtx")

def run_solver(precision, matrix_file, alpha):
    exe = os.path.join("exp1", EXECUTABLES[precision])
    # Command: CUDA_VISIBLE_DEVICES=4 ./exp1/gadi_bf16 <matrix> <alpha> <w>
...
def main():
    ...
    output_file = os.path.join("exp1", "search_results.txt")
