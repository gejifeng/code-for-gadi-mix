...
private:
    ...
    float *d_values, *d_x, *d_b, *d_r;
    
public:
    CuDSSSolver() : handle(nullptr), config(nullptr), data(nullptr), A_matrix(nullptr),
                    x_matrix(nullptr), b_matrix(nullptr), 
                    cusparse_handle(nullptr), cublas_handle(nullptr),
                    matrix_size(0), num_rows(0), num_cols(0), num_nnz(0),
                    d_row_ptr(nullptr), d_col_idx(nullptr), d_values(nullptr), 
                    d_x(nullptr), d_b(nullptr), d_r(nullptr) {}
    ...
    bool setupMatrix(const SparseMatrix& matrix) {
        ...
        CHECK_CUDA(cudaMalloc(&d_r, m * sizeof(float))); // 分配残差向量内存
        ...
        CHECK_CUDA(cudaMemcpy(d_values, matrix.values.data(), nnz * sizeof(float), cudaMemcpyHostToDevice));
        
        // 创建稀疏矩阵描述符
        CHECK_CUDSS(cudssMatrixCreateCsr(&A_matrix, m, n, nnz, d_row_ptr, nullptr, d_col_idx, d_values,
                                        CUDA_R_32I, CUDA_R_32F, CUDSS_MTYPE_GENERAL, CUDSS_MVIEW_FULL,
                                        CUDSS_BASE_ZERO));
    ...
    bool factorize() {
        ...
        CHECK_CUDSS(cudssMatrixCreateDn(&x_matrix, matrix_size, 1, matrix_size, d_x, CUDA_R_32F, CUDSS_LAYOUT_COL_MAJOR));
        CHECK_CUDSS(cudssMatrixCreateDn(&b_matrix, matrix_size, 1, matrix_size, d_b, CUDA_R_32F, CUDSS_LAYOUT_COL_MAJOR));
    ...
    bool solve(std::vector<float>& x, const std::vector<float>& b) {
        if (b.size() != x.size()) {
        ...
        // 复制结果回CPU
        CHECK_CUDA(cudaMemcpy(x.data(), d_x, n * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    float computeResidualNorm() {
        // 计算 r = b - Ax
        // 1. 复制 b 到 r
        CHECK_CUDA(cudaMemcpy(d_r, d_b, num_rows * sizeof(float), cudaMemcpyDeviceToDevice));
        ...
        CHECK_CUSPARSE(cusparseCreateDnVec(&vecR, num_rows, d_r, CUDA_R_32F));
        
        // SpMV 缓冲区
        size_t bufferSize = 0;
        void* dBuffer = nullptr;
        CHECK_CUSPARSE(cusparseSpMV_bufferSize(cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                               &alpha, matA, vecX, &beta, vecR, CUDA_R_32F,
        ...
        // 计算 r 的范数
        float r_norm = 0.0f;
        CHECK_CUBLAS(cublasSnrm2(cublas_handle, num_rows, d_r, 1, &r_norm));
        
        // 计算 b 的范数
        float b_norm = 0.0f;
        CHECK_CUBLAS(cublasSnrm2(cublas_handle, num_rows, d_b, 1, &b_norm));
        ...
        std::cout << "||b - Ax||_2 = " << r_norm << ", ||b||_2 = " << b_norm << std::endl;
...
std::vector<float> constructRightHandSide(const SparseMatrix& matrix) {
    int n = matrix.n;
    std::vector<float> x_true(n, 1.0f);  // 真解：全为1
    std::vector<float> b(n, 0.0f);       // 右端项：初始化为0
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // 创建右端向量（这里使用全1向量作为示例）
        std::cout << "\n=== 求解线性系统 ===" << std::endl;
        std::vector<float> b = constructRightHandSide(matrix);
        std::vector<float> x(matrix.n, 0.0f);
        ...
        // 计算相对残差
        std::cout << "\n=== 计算相对残差 ===" << std::endl;
        float rel_residual = solver.computeResidualNorm();
        std::cout << "相对残差: " << rel_residual << std::endl;
