...
def compile_code():
    ...
    subprocess.run(["make"], cwd="exp4", check=True)

def generate_matrix(n):
    print(f"Generating matrix for N={n}...")
    # python exp4/generate_A.py <N> <EPSILON>
    script_path = os.path.join("exp4", "generate_A.py")
    subprocess.run(["python", script_path, str(n)], check=True)
    return os.path.join("exp4", f"complex_diff_n{n}.mtx")

def run_solver(precision, matrix_file, alpha):
    exe = os.path.join("exp4", EXECUTABLES[precision])
    # Command: CUDA_VISIBLE_DEVICES=4 ./exp4/gadi_bf16 <matrix> <alpha> <w>
...
def main():
    ...
    output_file = os.path.join("exp4", "search_results.txt")
